...
SimulationResult run_simulation(const Gains& gains, bool write_output, bool run_visualization){
    ...
    // J = w_att*J_att/theta_att_ref^2
    //   + w_final*J_final/theta_final_ref^2
    //   + w_max*J_max/theta_max_ref^2
    //   + w_omega*J_omega/omega_ref^2
    //   + w_energy*J_energy/E_ref
    // For the current tuning stage, w_energy = 0.
    const double deg2rad = pi / 180.0;

    const double w_att    = 0.35;
    const double w_final  = 0.30;
    const double w_max    = 0.25;
    const double w_omega  = 0.10;
    const double w_energy = 0.00;

    const double theta_att_ref   = 5.0  * deg2rad;
    const double theta_final_ref = 2.0  * deg2rad;
    const double theta_max_ref   = 20.0 * deg2rad;
    const double omega_ref       = 0.5  * deg2rad;
    const double energy_ref      = 1.0; // Not active while w_energy = 0

    double cost = 0.0;
    double J_att_integral = 0.0;
    double J_omega_integral = 0.0;
    ...
    for(int t = 0; t < iteration_steps; t++){
        ...
        */
        ...
        // COMPUTE THE COST TERMS FOR THE SIMULATION
        // Attitude error angle from the error quaternion.
        // atan2(||qv||, |q0|) is used instead of acos(q0) for numerical robustness.
        double qv_norm = sqrt(quaternion_t2b[1] * quaternion_t2b[1]
                            + quaternion_t2b[2] * quaternion_t2b[2]
                            + quaternion_t2b[3] * quaternion_t2b[3]);

        double theta_err = 2.0 * atan2(qv_norm, fabs(quaternion_t2b[0]));
        ...
        // Accumulate integral terms. The division by simulation_time is done after the loop.
        J_att_integral += theta_err * theta_err * delta_time;
        J_omega_integral += omega2 * delta_time;

        // Store final attitude error angle in radians
        final_attitude_error = theta_err;

        // Store maximum attitude error angle in radians
        if(theta_err > max_attitude_error){
            max_attitude_error = theta_err;
        }

        // Store maximum angular rate in rad/s
        if(omega_norm > max_angular_rate){
            max_angular_rate = omega_norm;
        }

        // Detect numerical instability
        if(!std::isfinite(theta_err) || !std::isfinite(omega2)){
            stable = false;
            cost = 1e99;
            break;
        }
        ...
        // Reset actuator outputs for the current time step.
        // This avoids carrying power/current values from a previous step if a mode does not use a given actuator.
        for(int a = 0; a < 3; a++){
            magnetorquer_current[a] = 0.0;
            magnetorquer_power[a] = 0.0;
            magnetorquer_torque_body[a] = 0.0;
            magnetorquer_duty_cycle[a] = 0.0;
        }
        reactionWheel_current = 0.0;
        reactionWheel_power = 0.0;
        reactionWheel_torque = 0.0;
        ...
        // Accumulate actuator energy information.
        // This is returned for post-processing, but it does not affect the cost while w_energy = 0.
        double instantaneous_power = magnetorquer_power[0]
                                   + magnetorquer_power[1]
                                   + magnetorquer_power[2]
                                   + reactionWheel_power;

        total_energy += instantaneous_power * delta_time;
    ...
    if(stable){
        // Build dimensional cost terms
        double J_att = J_att_integral / simulation_time;
        double J_final = final_attitude_error * final_attitude_error;
        double J_max = max_attitude_error * max_attitude_error;
        double J_omega = J_omega_integral / simulation_time;
        double J_energy = total_energy;

        // Convert accumulated power sum into average power for output/post-processing
        total_power = total_energy / simulation_time;

        // Build final normalized cost
        cost = w_att    * J_att    / (theta_att_ref   * theta_att_ref)
             + w_final  * J_final  / (theta_final_ref * theta_final_ref)
             + w_max    * J_max    / (theta_max_ref   * theta_max_ref)
             + w_omega  * J_omega  / (omega_ref       * omega_ref)
             + w_energy * J_energy / energy_ref;
    }

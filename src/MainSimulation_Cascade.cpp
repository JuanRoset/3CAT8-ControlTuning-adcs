...
// The original simulation used one attitude gain vector Kp[3]
// and one angular-rate damping gain vector Kd[3].
// The cascade controller separates the control into two loops:
//   1) attitude/angle-error loop -> target angular velocity
//   2) angular-rate loop         -> target control torque
double Kp_phi[3];
double Kd_phi[3];
double Kp_omega[3];
double Kd_omega[3];

// Temporary arrays only used to keep compatibility with satellite_fetch(...),
// which still reads the old Kp/Kd values from the JSON file.
double Kp_config[3];
double Kd_config[3];
...
void cascade_control_torque(double error_quaternion[4],
                            double Kp_phi[3],
                            double Kd_phi[3],
                            double measured_angular_rate[3],
                            double Kp_omega[3],
                            double Kd_omega[3],
                            double delta_time,
                            double previous_attitude_error[3],
                            double previous_omega_error[3],
                            bool& derivative_initialized,
                            double target_angular_rate[3],
...
// Main simulation callable by MonteCarlo_Cascade.cpp

SimulationResult run_simulation_cascade(const CascadeGains& gains, bool write_output, bool run_visualization){
    ...
    inertia_wheels[2][2] = reactionWheel.mainInertia;

    // Fetch all the satellita and sim parameters
    satellite_fetch(inertia, inertia_principal_min, control_mode, Kp_config, Kd_config, RW_relaxation, Ku, active_force_control, RW_unloading, configuration, satelliteParamFilePath);

    for(int i = 0; i < 3; i++){
        Kp_phi[i]   = gains.Kp_phi[i];
        Kd_phi[i]   = gains.Kd_phi[i];
        Kp_omega[i] = gains.Kp_omega[i];
        Kd_omega[i] = gains.Kd_omega[i];
    }
    ...
    if(write_output){
        outFile.open(filePathOutput);

        if (!outFile.is_open()) {
            std::cerr << "Error opening the file: " << filePathOutput << std::endl;
            return {1e99, 0.0, 0.0, 0.0, false};
        }
    ...
    // Cascade controller memory. The derivatives in the two cascade loops
    // are estimated with finite differences from the previous time step.
    double previous_attitude_error[3]        = {0.0};
    double previous_omega_error[3]           = {0.0};
    double target_angular_rate_body[3]       = {0.0};
    bool cascade_derivative_initialized      = false;
    ...
    for(int t = 0; t < iteration_steps; t++){
        ...
        normalize_quaternion(quaternion_t2b);

        // Force shortest quaternion representation: q and -q represent the same attitude
        if(quaternion_t2b[0] < 0.0){
            for(int i = 0; i < 4; i++){
                quaternion_t2b[i] = -quaternion_t2b[i];
            }
        }
        ...
        */
        ...
        // COMPUTE THE COST FOR SIMULATION
        // Compute attitude error using the vector part of the error quaternion
        double qv2 = quaternion_t2b[1] * quaternion_t2b[1]
           + quaternion_t2b[2] * quaternion_t2b[2]
           + quaternion_t2b[3] * quaternion_t2b[3];

        // Compute relative angular velocity error
        double omega2 = angular_rate_o2b_body[0] * angular_rate_o2b_body[0]
              + angular_rate_o2b_body[1] * angular_rate_o2b_body[1]
              + angular_rate_o2b_body[2] * angular_rate_o2b_body[2];

        double current_time = t * delta_time;

        // Time weighting: later errors are penalized more strongly
        double time_weight = 2.0 * current_time / simulation_time;

        // Main tuning cost
        cost += time_weight * qv2 * delta_time;

        // Optional: small penalty on angular velocity
        cost += 0.05 * time_weight * omega2 * delta_time;

        // Store final attitude error
        final_attitude_error = sqrt(qv2);

        // Store maximum angular rate
        double omega_norm = sqrt(omega2);
        if(omega_norm > max_angular_rate){
            max_angular_rate = omega_norm;
        }

        // Detect numerical instability
        if(!std::isfinite(qv2) || !std::isfinite(omega2)){
            stable = false;
            cost = 1e99;
            break;
        }
        ...
        if(control_mode == "ideal"){
            // Calculate the ideal cascade control torque in body axes
            cascade_control_torque(quaternion_t2b, Kp_phi, Kd_phi, angular_rate_o2b_body,
                                   Kp_omega, Kd_omega, delta_time,
                                   previous_attitude_error, previous_omega_error,
                                   cascade_derivative_initialized,
                                   target_angular_rate_body, target_torque_body);
    ...
    if(stable){
        cost = cost / simulation_time;
    }

    return {cost, final_attitude_error, max_angular_rate, total_power, stable};
}

//TOCA HACER EL MAIN DE MONTE CARLO
...
void cascade_control_torque(double error_quaternion[4],
                            ...
                            double target_angular_rate[3],
                            double torque_body[3]){
    // Cascaded PD controller for pointing.
    //
    // Step 1: attitude/angle-error loop
    //   error_quaternion[1:3] is used as the small-angle attitude-error proxy.
    //   The output is the desired relative angular velocity in body axes.
    //
    // Step 2: angular-rate loop
    //   The measured angular velocity is compared with that desired angular velocity.
    //   The output is the ideal control torque in body axes.
    //
    // Sign convention:
    //   quaternion_t2b is computed as q_target^{-1} * q_body in this codebase.
    //   With that convention, the corrective angular-rate command must have the
    //   opposite sign to the quaternion vector part. This keeps the physical
    //   behaviour equivalent to the original simple PD controller.
    ...
    double attitude_error_derivative[3] = {0.0};
    double omega_error[3] = {0.0};
    double omega_error_derivative[3] = {0.0};

    for(int i = 0; i < 3; i++){
        if(derivative_initialized && delta_time > 0.0){
            attitude_error_derivative[i] = (attitude_error[i] - previous_attitude_error[i]) / delta_time;
        }

        // First cascade loop:
        // desired relative angular velocity generated from attitude error.
        target_angular_rate[i] = - (Kp_phi[i] * attitude_error[i]
                                  + Kd_phi[i] * attitude_error_derivative[i]);

        // Second cascade loop:
        // angular-rate error is measured minus target, so target = 0 recovers
        // a damping law against unwanted angular velocity.
        omega_error[i] = measured_angular_rate[i] - target_angular_rate[i];

        if(derivative_initialized && delta_time > 0.0){
            omega_error_derivative[i] = (omega_error[i] - previous_omega_error[i]) / delta_time;
        }

        torque_body[i] = - (Kp_omega[i] * omega_error[i]
                          + Kd_omega[i] * omega_error_derivative[i]);
    }

    for(int i = 0; i < 3; i++){
        previous_attitude_error[i] = attitude_error[i];
        previous_omega_error[i] = omega_error[i];
    }

...
// Best gains found with the optimisation run shown in the terminal.
// These values override the gains read from ../parameters/satellite_parameters.json
// so this standalone main can reproduce the best case directly.
const bool USE_BEST_OPTIMISED_GAINS = true;
const double BEST_KP[3] = {1.28784e-06, 3.33282e-06, 9.81425e-07};
const double BEST_KD[3] = {4.79187e-05, 0.000370398, 6.3963e-05};
...
int main(){
    ...
    // The reaction wheel inertia is around the body Z axis.
    // Original code had inertia_wheels[3][3], which is out of bounds for a 3x3 array.
    inertia_wheels[2][2] = reactionWheel.mainInertia;

    // Fetch all the satellite and simulation parameters.
    // Kp and Kd are first read from ../parameters/satellite_parameters.json.
    ...
    // Override the JSON gains with the best optimised gains, if enabled.
    if(USE_BEST_OPTIMISED_GAINS){
        for(int i = 0; i < 3; i++){
            Kp[i] = BEST_KP[i];
            Kd[i] = BEST_KD[i];
        }
    ...
    for(int t = 0; t < iteration_steps; t++){
        ...
        // q and -q represent the same attitude. Force the shortest representation
        // so the cost function is consistent.
        ...
        */
        ...
        // ================================================================
        // Cost function
        // It penalizes attitude error through the vector part of the error
        // quaternion and adds a small penalty on relative angular velocity.
        // The time_weight gives more importance to errors near the end of
        // the simulation.
        // ================================================================
        ...
        if(!std::isfinite(qv2) || !std::isfinite(omega2)){
            stable = false;
            cost = 1e99;
            std::cerr << "Numerical instability detected. Simulation stopped." << std::endl;
            break;
        }
    ...
    std::cout << "Final attitude error: " << final_attitude_error << std::endl;
    std::cout << "Max angular rate: " << max_angular_rate << std::endl;

    if(USE_BEST_OPTIMISED_GAINS){
        std::cout << "Gains source: hard-coded best optimised gains" << std::endl;
    }
    else{
        std::cout << "Gains source: ../parameters/satellite_parameters.json" << std::endl;
    }

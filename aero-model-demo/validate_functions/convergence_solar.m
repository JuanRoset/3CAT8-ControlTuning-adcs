...
delta_array = expspace(0.25, 0.0025, 30)';
...
cos_theta_a = abs(dot(normal_a, ray));
cos_theta_b = abs(dot(normal_b, ray));
cos_theta_c = abs(dot(normal_c, ray));

% Compute aerodynamic force components
N = - ray;
Fs_a = - P * area_a * cos_theta_a .* ((1 - rho_s) * N + 2 * (rho_s * cos_theta_a + rho_d / 3) * normal_a);
Fs_b = - P * area_b * cos_theta_b .* ((1 - rho_s) * N + 2 * (rho_s * cos_theta_b + rho_d / 3) * normal_b);
Fs_c = - P * area_c * cos_theta_c .* ((1 - rho_s) * N + 2 * (rho_s * cos_theta_c + rho_d / 3) * normal_c);

Fs_a = - P * area_a * dot(normal_a, ray) .* ((1 - rho_s) * ray + 2 * (rho_s * dot(normal_a, ray) - rho_d / 3) * normal_a);
Fs_b = - P * area_b * dot(normal_b, ray) .* ((1 - rho_s) * ray + 2 * (rho_s * dot(normal_b, ray) - rho_d / 3) * normal_b);
Fs_c = - P * area_c * dot(normal_c, ray) .* ((1 - rho_s) * ray + 2 * (rho_s * dot(normal_c, ray) - rho_d / 3) * normal_c);

% Compute aerodynamic torques of all plates
r_a = p_a - position_CG;
r_b = p_b - position_CG;
r_c = p_c - position_CG;
tau_a = cross(r_a, Fs_a);
tau_b = cross(r_b, Fs_b);
tau_c = cross(r_c, Fs_c);

% Compute total forces and torques
F_analytical = (Fs_a + Fs_c + Fs_b)';
...
for n = 1:size(delta_array, 1)
    ...
    for i = 1:size(direct_points, 1)
        ...
        F = - P * direct_areas(i) * dot(A_unit, ray) .* ((1 - rho_s) * ray + 2 * (rho_s * dot(A_unit, ray) - rho_d / 3) * A_unit);

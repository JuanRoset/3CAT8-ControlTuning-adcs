...
/************************************************************************
	 ...
	 Sponsoring Government Agency
      ...
      4/5/05 Update - Pau Climent:
      - Changes to the code structure for it to properly run in the simulator
...
void igrf_field(double dlat, double dlon, double altm, double time, double mag_field[3], const char *wmm_path) {
  ...
  // · dlong: longitude in degrees
  // · altm: altitude in meters
  // · time: decimal year
  // Outputs:
  // · mag_field[3]: magnetic field values in X Y Z components wrt the NED (North, East, Down) frame

  altm /= 10e3; // Convert altitude to km
...
static void E0000(int IENTRY, int *maxdeg, double alt, double glat, double glon, double time, double *dec, double *dip, double *ti, double *gv, const char *wmm_path)
{
  ...
  static double a2,b2,c2,a4,b4,c4,epoch,gnm,hnm,dgnm,dhnm,flnmj,otime,oalt;
  static double olat,olon,dt,rlon,rlat,srlon,srlat,crlon,crlat,srlat2;
  static double crlat2,q,q1,q2,ct,st,r2,r,d,ca,sa,aor,ar,br,bt,bp,bpp;
  static double par,temp1,temp2,parp,bx,by,bz,bh;
